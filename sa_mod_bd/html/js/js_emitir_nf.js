document.addEventListener("DOMContentLoaded", function () {
  ...
  window.preencherFormulario = function (item) {
    const campos = [
      "id_pecas", "nome", "aparelho_utilizado",
      "quantidade", "preco", "data_registro", "numero_serie", "descricao"
    ];
  ...
  if (pesquisarBtn) {
    ...
    window.excluirItem = function(item) {
      Swal.fire({
        title: 'Tem certeza?',
        text: `Você está prestes a excluir a NF "${item.nome}" (ID: ${item.id_pecas})`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
